...
function WorkspaceIcon({ item, className = "size-[18px]" }: { item: DashboardTab; className?: string }) {
  return (
  ...
  return (
    <main className="dashboard-shell min-h-screen bg-[#fbfaf7] text-courier-ink">
      <header className="sticky top-0 z-40 border-b border-courier-line bg-white/95 backdrop-blur-md">
        <div className="mx-auto flex min-h-[76px] max-w-[1380px] items-center justify-between gap-5 px-5 sm:px-8 lg:px-12">
          <BrandMark />
      ...
      <div className="mx-auto max-w-[1540px] lg:grid lg:grid-cols-[238px_minmax(0,1fr)]">
        <aside className="dashboard-sidebar sticky top-[76px] hidden h-[calc(100dvh-76px)] flex-col bg-white px-5 py-7 lg:flex" aria-label="Main navigation">
        ...
        <section className="dashboard-welcome grid gap-6 py-7 sm:py-9 lg:grid-cols-[minmax(0,1fr)_minmax(290px,.62fr)] lg:items-center">
          <div>
            ...
            <h1 className="mb-0 max-w-[800px] font-display text-[clamp(2.4rem,4.5vw,4rem)] font-semibold leading-[1] tracking-[-.06em]">Good to see you, <em className="font-normal text-courier-green">{userName.split(" ")[0]}.</em></h1>
            <p className="mb-0 mt-4 max-w-[520px] text-[14px] leading-6 text-courier-muted">Requests, routes and handoffs, all moving from one place.</p>
          </div>
          <div className="relative overflow-hidden rounded-[26px] border border-[#d9e5dc] bg-[#edf4ef] px-4 py-4 sm:px-5">
            <div className="flex items-center justify-between gap-4">
              ...
              <svg className="h-[42px] w-[92px] shrink-0" viewBox="0 0 92 42" fill="none" aria-hidden="true"><path d="M5 31C22 31 17 9 39 9s17 24 37 24 11-17 12-17" stroke="#bd8845" strokeWidth="1.5" strokeDasharray="3 4"/><circle cx="5" cy="31" r="4" fill="#145b4c"/><rect x="84" y="12" width="7" height="7" fill="#bd8845"/></svg>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button className="min-h-[42px] rounded-full bg-courier-green px-3 text-[10px] font-semibold text-white transition-colors hover:bg-courier-green-deep" type="button" onClick={() => { setFormOpen(formOpen === "request" ? null : "request"); setTab("requests"); }}>Create a request</button>
              <button className="min-h-[42px] rounded-full border border-courier-green px-3 text-[10px] font-semibold text-courier-green transition-colors hover:bg-white" type="button" onClick={() => { setFormOpen(formOpen === "journey" ? null : "journey"); setTab("journeys"); }}>Share a route</button>
            </div>
          </div>
        </section>

        <section className="grid grid-cols-3 gap-2 py-3 sm:gap-4 sm:py-4" aria-label="Workspace summary">
          <div className="rounded-[20px] border border-courier-line bg-white px-3 py-3.5 sm:px-5 sm:py-4"><span className="block text-[8px] font-bold uppercase tracking-[.1em] text-courier-muted sm:text-[10px]">Open requests</span><strong className="mt-1 block font-display text-[27px] font-semibold tracking-[-.05em] sm:text-[32px]">{requests.filter(({ status }) => ["OPEN", "MATCHED"].includes(status)).length.toString().padStart(2, "0")}</strong></div>
          <div className="rounded-[20px] border border-courier-line bg-white px-3 py-3.5 sm:px-5 sm:py-4"><span className="block text-[8px] font-bold uppercase tracking-[.1em] text-courier-muted sm:text-[10px]">Upcoming routes</span><strong className="mt-1 block font-display text-[27px] font-semibold tracking-[-.05em] sm:text-[32px]">{itineraries.filter(({ status }) => status === "PLANNED" || status === "ACTIVE").length.toString().padStart(2, "0")}</strong></div>
          <div className="rounded-[20px] border border-courier-line bg-white px-3 py-3.5 sm:px-5 sm:py-4"><span className="block text-[8px] font-bold uppercase tracking-[.1em] text-courier-muted sm:text-[10px]">Active agreements</span><strong className="mt-1 block font-display text-[27px] font-semibold tracking-[-.05em] sm:text-[32px]">{agreements.filter(({ status }) => !["COMPLETED", "CANCELLED"].includes(status)).length.toString().padStart(2, "0")}</strong></div>
        </section>

        <nav className="dashboard-mobile-nav fixed inset-x-3 bottom-3 z-30 grid grid-cols-4 rounded-[24px] border border-courier-line bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden" aria-label="Workspace views">
          {(["overview", "requests", "journeys", "agreements"] as const).map((item) => (
            <button key={item} className={`relative flex min-h-[62px] flex-col items-center justify-center gap-1 text-[9px] font-semibold capitalize transition-colors ${tab === item ? "text-courier-green" : "text-courier-muted hover:text-courier-ink"}`} type="button" aria-current={tab === item ? "page" : undefined} onClick={() => { setTab(item); setFormOpen(null); }}>
              <WorkspaceIcon item={item} className="size-[18px]" />
          ...
          <div className="grid gap-x-12 gap-y-14 pt-9 lg:grid-cols-2">
            {visibleRequests && <section className={tab === "requests" ? "lg:col-span-2" : ""} aria-labelledby="requests-heading">
              ...
              {formOpen === "request" && <form className="mb-5 grid gap-4 border border-courier-line p-4 sm:grid-cols-2 sm:p-6" onSubmit={submitRequest}>
                ...
                <label className={`${labelClass} sm:col-span-2`}>A little more detail<textarea className={`${fieldClass} min-h-[86px] py-3`} name="description" maxLength={4000} placeholder="Tell the traveler what it is and anything they should know." required /></label>
                ...
                <div className="flex items-end"><button className="min-h-[46px] w-full bg-courier-green px-4 text-[10px] font-semibold text-white transition-colors hover:bg-courier-green-deep disabled:opacity-60" type="submit" disabled={submitting}>{submitting ? "Saving…" : "Post this request"}</button></div>
              </form>}
            ...
            {visibleJourneys && <section className={tab === "journeys" ? "lg:col-span-2" : ""} aria-labelledby="journeys-heading">
              ...
              {formOpen === "journey" && <form className="mb-5 grid gap-4 border border-courier-line p-4 sm:grid-cols-2 sm:p-6" onSubmit={submitItinerary}>
                ...
                <div className="sm:col-span-2"><button className="min-h-[46px] bg-courier-green px-5 text-[10px] font-semibold text-white transition-colors hover:bg-courier-green-deep disabled:opacity-60" type="submit" disabled={submitting}>{submitting ? "Saving…" : "Share this route"}</button></div>
              </form>}
...
function EmptyState({ title, body, action, onAction }: { title: string; body: string; action?: string; onAction?: () => void }) {
  return <div className="border border-dashed border-courier-line px-5 py-8 sm:px-7"><span className="font-display text-[13px] italic text-courier-gold">A fresh page</span><h3 className="mb-2 mt-3 font-display text-[23px] tracking-[-.04em]">{title}</h3><p className="mb-0 max-w-[430px] text-[11px] leading-5 text-courier-muted">{body}</p>{action && onAction && <button className="mt-5 text-[10px] font-semibold text-courier-green underline decoration-courier-line underline-offset-4 hover:decoration-courier-green" type="button" onClick={onAction}>{action} →</button>}</div>;
}
...
function RequestCard({ item, panel, loading, onMatches, onStartAgreement, onUpdate, onCancel, submitting }: { item: ItemRequest; panel?: MatchPanel; loading: boolean; onMatches: () => void; onStartAgreement: (matchId: string) => void; onUpdate: (event: FormEvent<HTMLFormElement>, item: ItemRequest) => void; onCancel: (id: string) => void; submitting: boolean }) {
  ...
  return <article className="group border border-courier-line bg-white px-4 py-4 transition-colors hover:border-[#c6d4ca] sm:px-5 motion-safe:animate-enter">
    ...
    {editing && <form className="mt-4 grid gap-3 border-y border-courier-line py-4 sm:grid-cols-2" onSubmit={(event) => { onUpdate(event, item); setEditing(false); }}>
      ...
      <label className={`${labelClass} sm:col-span-2`}>Description<textarea className={`${fieldClass} min-h-[72px] py-3`} name="description" maxLength={4000} defaultValue={item.description} required /></label>
      ...
      <div className="flex items-end gap-3"><button className="min-h-[42px] bg-courier-green px-4 text-[10px] font-semibold text-white disabled:opacity-60" type="submit" disabled={submitting}>Save changes</button><button className="min-h-[42px] border border-courier-line px-4 text-[10px] font-semibold text-courier-muted" type="button" onClick={() => setEditing(false)}>Keep as is</button></div>
    </form>}
...
function ItineraryCard({ item, panel, loading, onMatches, onRespond, onUpdate, onCancel, submitting }: { item: Itinerary; panel?: MatchPanel; loading: boolean; onMatches: () => void; onRespond: (matchId: string, itineraryId: string) => void; onUpdate: (event: FormEvent<HTMLFormElement>, item: Itinerary) => void; onCancel: (id: string) => void; submitting: boolean }) {
  ...
  return <article className="group border border-courier-line bg-white px-4 py-4 transition-colors hover:border-[#c6d4ca] sm:px-5 motion-safe:animate-enter">
    ...
    {editing && <form className="mt-4 grid gap-3 border-y border-courier-line py-4 sm:grid-cols-2" onSubmit={(event) => { onUpdate(event, item); setEditing(false); }}>
      ...
      <label className={`${labelClass} sm:col-span-2`}>Stops along your route · optional<textarea className={`${fieldClass} min-h-[76px] py-3`} name="routeStopsText" maxLength={4000} defaultValue={(item.routeStops ?? []).map(({ name, countryCode }) => `${name}, ${countryCode}`).join("\n")} placeholder={"Ibadan, NG\nAccra, GH"} /><span className="mt-1.5 block text-[9px] font-normal normal-case tracking-normal text-courier-muted">One city per line, followed by its country code.</span></label>
      <div className="flex items-end gap-3 sm:col-span-2"><button className="min-h-[42px] bg-courier-green px-4 text-[10px] font-semibold text-white disabled:opacity-60" type="submit" disabled={submitting}>Save route</button><button className="min-h-[42px] border border-courier-line px-4 text-[10px] font-semibold text-courier-muted" type="button" onClick={() => setEditing(false)}>Keep as is</button></div>
    </form>}
